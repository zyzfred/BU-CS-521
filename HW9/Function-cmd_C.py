...
def cmd_C(start, end, z, delta):
	if ord(z[DATA][delta]) >= 97 and ord(z[DATA][delta]) <= 122:
		capital_char = chr(ord(z[DATA][delta])- 32)
		z[DATA] = z[DATA][:delta] + capital_char + z[DATA][delta + 1:]
...
new_start, new_end, z, delta = cmd_C(new_start, new_end, z, delta)
print_file(new_start, new_end, z, delta)