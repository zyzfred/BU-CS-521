...
def cmd_j(start, end, z, delta): # vertically up
    if z != start:
        # if delta > length of previous line
        curline = z[DATA]
        preline = z[P_PTR][DATA]
        if delta > len(preline):
            detla = len(preline) - 1
        z = z[P_PTR]
...
new_start, new_end, z, delta = cmd_j(new_start, new_end, z, delta)
print_file(new_start, new_end, z, delta)